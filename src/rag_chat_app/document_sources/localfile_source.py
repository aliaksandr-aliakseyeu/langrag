...
from rag_chat_app.utils import get_file_hash
...
class LocalfileSource(DocumentSource):
    def __init__(self, storage_path: str, supported_extensions: List[str] = None):
        ...
        self.supported_extensions = supported_extensions or [
            '.txt', '.md', '.pdf', '.docx', '.doc', '.html', '.htm', '.csv', '.xlsx', '.rtf'
        ]
    ...
    def list_documents(self) -> List[DocumentMetadata]:
        ...
        for file_path in self._get_files():
            try:
                ...
                print(f'Oops! Unexpected error occurred while processing {file_path}: {e}')
                continue
    ...
    def _get_files(self, recursive: bool = True) -> Iterator[Path]:
        files = self.storage_path.rglob('*') if recursive else self.storage_path.glob('*')
